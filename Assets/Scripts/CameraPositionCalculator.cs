...
public class CameraPositionCalculator : MonoBehaviour
{
   ...
   public void CameraDivergence(Vector2 coordinate)
   {
      ...
      mainCamera.orthographicSize = cameraSettings.divergenceValue;
      mainCamera.orthographicSize +=  Mathf.Max(coordinate.y, coordinate.x) * 
                                     ((1920 / 1080f) / (mainCamera.pixelWidth / (mainCamera.pixelHeight * 1f))) / 2f;
   }
